      ...
      sections: [
        {
          ...
          prose:
            "`ProgramArguments` resolves `gomi` from the Nix store via `lib.getExe` — no `$PATH` dependency. `StartCalendarInterval` runs the prune every Sunday at 03:00, logging to `~/Library/Logs/gomi-prune.log` rather than `/tmp`, which is cleared on reboot; the agent is owned by the user, so it doesn't need root.",
          lines: [8, 29],
        },
      ],
    },
