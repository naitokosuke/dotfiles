...
export const explanations: Readonly<Record<string, Explanation>> = {
  ...
  "home/claude.nix": {
    ...
    walkthrough: {
      ...
      sections: [
        {
          title: "Cask-provided binaries, checked at eval time",
          prose:
            "Two hooks in this file run binaries a Homebrew cask installs, so their paths cannot come from the store. `requireCask` ties each reference to the cask that provides it: drop `adrafinil` or `visual-studio-code` from `hosts/common/homebrew.nix` and evaluation fails here, naming the file and the cask, instead of leaving a hook that silently does nothing (issue #482). nix-darwin normalises `homebrew.casks` into submodules, so the check reads each entry's `name`.",
          lines: [9, 20],
        },
        {
  ...
  "home/mcp.nix": {
    ...
    walkthrough: {
      intro:
        "MCP (Model Context Protocol) lets Claude Code talk to external tools. `natsukium/mcp-servers-nix` provides the registry through its home-manager module (injected via `sharedModules`), `programs.mcp` collects the servers, and `enableMcpIntegration` hands them to Claude Code. The one server, Adrafinil's `keep_awake`, isn't covered by the registry's built-in modules, so it slots in via the `settings.servers` freeform escape hatch and runs the CLI inside the Homebrew-installed app bundle. It lets an agent keep the Mac awake for work that outlives its turn, which the hooks in `home/claude.nix` do not cover. That bundle path can't come from the store, so it's guarded instead: removing the `adrafinil` cask from `hosts/common/homebrew.nix` fails evaluation rather than leaving a server entry pointing at nothing (issue #482).",
    },
  },
    ...
    walkthrough: {
      ...
      sections: [
        ...
        },
        {
          ...
          prose:
            "Starship has no built-in module for disk space, so `diskFree` builds a `custom` module that runs `/bin/df -h /` and prints a hard disk glyph followed by the available space over the disk size, as in `with 󰋊 120Gi / 500Gi`. The macOS system and data volumes share one APFS container, so `/` reports the same free space as the data volume. `df` and `awk` are macOS's own and spelled out in full: the awk programs read the column layout of BSD `df`, which GNU coreutils' would not reproduce, so this is one place where staying out of the store is the right answer and the comment says so (issue #482). `sh`, which only runs a pipeline and a test, is pinned to the store instead — the prompt then depends on no `$PATH` ordering at all.",
          lines: [3, 26],
        },
        {
